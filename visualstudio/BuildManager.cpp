    ...
    for (ResourceRequest& request : resourceRequests)
    {
        ...
        switch (request.type)
        {
            ...
            case ResourceRequest::Type::Building:
            {
                ...
                {  
                    // for natural walling, bypass PlacementInfo and use useForcedTile to build exactly there.
...
std::string BuildManager::buildOrderNameToString(int name) const
{
    switch (name)
    {
        ...
        case 3: return "9/9 Gateways vs. Zerg";
...
bool BuildManager::enqueueNaturalWallAtChoke()
{
    ...
    if (!naturalWallPlanned)
    {
        ...
        if (!wall->getSmallTiles().empty())
        {
            ...
            // If still invalid, fall back to findNaturalChokePylonTile() later
