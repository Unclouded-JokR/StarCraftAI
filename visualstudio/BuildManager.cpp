...
bool BuildManager::enqueueNaturalWallAtChoke()
{
    ...
    if (!naturalWallPlanned)
    {
        ...
        for (std::vector<std::vector<BWAPI::UnitType>>::iterator it = candidates.begin(); it != candidates.end(); ++it)
        {
            ...
            // First anchor the wall on the choke anchor, then keep the A* corridor as the gap.
        ...
        // Classify small wall pieces: pick the pylon tile closest to the A* gap, treat remaining small tiles as cannons.
