...
{
	this->commanderReference = commander;
}
...
bool InformationManager::isEnemyEarlyPush() const
{
	...
	for (const auto& kv : trackedEnemies)
	{
		...
		if (t.isValid() && t.isBuilding())
		{
			++enemyBuildings;
			// Check if this cannon is near any of our bases
	...
	if (frame <= 10000)
	{
		...
		if (buildingsNearBase >= 1) return true;
	}
